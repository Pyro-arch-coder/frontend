...
export default function SNewChildRequest() {
  ...
  const barangayList = ['All', ...santaMariaBarangays.Barangays];
  const [actionResult, setActionResult] = useState({ open: false, type: '', message: '' });

  const fetchRequests = async () => {
    setLoading(true);
    try {
      // For superadmin, fetch all new child requests (no barangay filter)
      const res = await axios.get(`${API_BASE_URL}/newchildrequest/all`);
      let requests = [];
      if (Array.isArray(res.data.requests)) {
        requests = res.data.requests.filter(r => r.status === 'Approved');
      }
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      // Use superadmin endpoints
      let url;
      if (action === 'approve') {
        url = `${API_BASE_URL}/newchildrequest/superadmin-approve`;
      } else if (action === 'decline') {
        url = `${API_BASE_URL}/newchildrequest/superadmin-decline`;
      } else {
            ...
            <Box sx={{
              ...
              <Box sx={{ minWidth: 180 }}>
                <TextField
                  ...
                  size="small"
                >
                  {barangayList.map((barangay, idx) => (
                    <option key={idx} value={barangay}>{barangay}</option>
                  ))}
                </TextField>
              </Box>
            </Box>
